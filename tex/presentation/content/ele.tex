		...
		\begin{column}{0.5\textwidth}
			\begin{figure}
				\begin{tikzpicture}[node distance=1.5cm]
					...
					\draw[blue, thick, <->] (linux) |- node[anchor=north] {/dev/tty*} (usb);
					\draw[blue, thick, <->] (windows) -- node[anchor=east] {COM} (usb);
					\draw[blue, thick, <->] (mac) |- node[anchor=north] {/dev/cu.*} (usb);
					\draw[blue, thick, <->] (usb) -- (frdm);
					\draw[blue, thick, <->] (frdm) -| node[anchor=east, very near end] {SPI} (x);
					\draw[blue, thick, <->] (frdm) -- node [anchor=east] {I$^2$C} (y);
					\draw[blue, thick, <->] (frdm) -| node[anchor=west, very near end] {UART} (z);
				\end{tikzpicture}
			\end{figure}
		\end{column}
		\begin{column}{0.5\textwidth}
