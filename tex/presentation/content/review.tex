...
\begin{frame}
	...
	\begin{figure}
		\begin{tikzpicture}[node distance=2.85cm, scale=0.75, transform shape]
			...
			\draw[blue, thick, ->] (ov) -| node[anchor=south, very near start] {Y} (a);
			\draw[blue, thick, ->] (ov) -- node[anchor=south, very near start] {N} (si);
			\draw[blue, thick, ->] (si) -- (is);
			\draw[blue, thick, ->] (is) -- node[anchor=south, very near start] {Y} (ci);
			\draw[blue, thick, ->] (is) -| node[anchor=south, very near start] {N} (np);
			\draw[blue, thick, ->] (np) -| node[anchor=south, very near start] {N} (a);
			\draw[blue, thick, ->] (np) -- node[anchor=west, very near start] {Y} (i);
		\end{tikzpicture}
	\end{figure}
\end{frame}
